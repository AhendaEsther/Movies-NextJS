...
const Input: React.FC<InputProps> = ({ type = "text", placeholder, value,onChange,className = "",Icon,onIconClick,}) => {
  return (
    <div className={`flex items-center border border-gray-200 rounded px-3 py-1 bg-white ${className}`}>
      <input type={type} placeholder={placeholder} value={value} onChange={onChange} className="flex-grow outline-none text-black bg-"/>
      {Icon && (
